...
app.post("/api/mapClick", (req, res) => {
    ...
    let bigCircle = turf.circle([long, lat], 0.5, {
        steps: 10,
        units: "miles",
    });

    let meters = JSON.parse(
        fs.readFileSync(
            path.join(__dirname, "../data/data/parking_meters.geojson"),
            "utf8"
        )
    );
    let metersInCircle = {
    ...
    meters.features.forEach((meter) => {
        if (
            turf.booleanContains(
                bigCircle,
                turf.point(meter.geometry.coordinates)
            )
        ) {
    ...
    //get all violations
    let violations = JSON.parse(
        fs.readFileSync(
            path.join(__dirname, "../data/data/master_violations.geojson"),
            "utf-8"
        )
    );
    ...
    violations.features.forEach((violation) => {
        if (
            turf.booleanContains(
                bigCircle,
                turf.point(violation.geometry.coordinates)
            )
        ) {
            violationsInCircle.features.push(violation);
            violationCircles.features.push(
                turf.circle(turf.point(violation.geometry.coordinates), 0.01, {
                    steps: 10,
                    units: "miles",
                })
            );
    ...
    console.log(
        `Meters: ${metersInCircle.features.length}, Violations: ${violationCircles.features.length}`
    );
